...
//uint32_t Kernel_task_create(KernelTaskFunc_t startFunc, uint32_t priority){ // Priority applied
	...
	//new_tcb->priority = priority;	// assign priority to task control block
...
static __attribute__ ((naked)) void Restore_context(void){
	...
	__asm__ ("LDR r0, [r0]");
	__asm__ ("LDMIA r0!, {sp}");
	// restore next task context from next task stack
	__asm__ ("POP {r0}");
	__asm__ ("MSR cpsr, r0");
